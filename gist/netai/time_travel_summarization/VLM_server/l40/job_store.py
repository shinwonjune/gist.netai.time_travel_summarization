...
설계: docs/잡스토어_설계.md. 현재 job_api는 잡 상태를 파일(status)에, 대기 큐를
프로세스 메모리(queue.Queue)에 둔다. 이 모듈은 그 둘을 하나의 JobStore 추상화 뒤로
옮겨 **데몬 재시작에도 잡이 보존**되게 하고, 단일 노드(SQLite)와 다중 노드(Postgres)를
같은 인터페이스로 지원한다.
