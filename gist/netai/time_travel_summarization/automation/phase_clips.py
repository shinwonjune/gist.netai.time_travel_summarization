...
= 2r)보다 넉넉히 크게 잡은 값이다 — contact report는 중심 거리가 2r에 닿는 순간
발화하므로 거리 시계열의 최저값이 2r을 크게 밑돌지 않고, 문턱을 2r에 딱 맞추면
샘플링 격자 탓에 하회 순간을 놓친다. **2r은 규약 세대마다 다르다**: v1 71.7 /
...
# --- 접촉 구간 실측 --------------------------------------------------------
DEFAULT_TOUCH_THRESHOLD = 90.0   # 지면 거리 문턱(cm). v1 규약의 2r≈71.7보다 여유를 둔 값
                                 # (regime2 2r=63.7 / regime3 2r=60.0 — 모듈 docstring 참조)
...
# 것은 거리가 최솟값 부근(v1 실측 71~88cm)에 도달한 뒤다. 사건마다 d_min이 달라
# 고정 거리 가드로는 이 경계를 잡을 수 없고, 사건별 d_min에 상대적인 여유로 잡아야
# 한다. d_min이 흔들리는 이유는 규약 세대마다 다르다: v1/regime2는 콜라이더 반지름이
# 스폰 각도에 좌우돼 접촉 거리 자체가 사건마다 달랐고, regime3는 반지름 30 상수라
# 접촉 거리가 60으로 고정되는 대신 이산 스텝 솔버의 침투가 남아 d_min이 56~62로
# 흔들린다(prod-20260817-regime3 실측). 어느 쪽이든 상대 여유 방식이 옳다.
