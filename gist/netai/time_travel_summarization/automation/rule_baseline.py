...
#
# regime3 규약(접촉거리 60.0) 기준 = **69.0** — 2026-08-20 재동결.
#   측정: prod-20260817-regime3의 held-out(v5 test 분할) 75 에피소드 / GT 160 이벤트,
#         τ 54~72를 0.5 단위 스윕. 후보를 접촉거리 60 중심으로 좁힌 이유는 아래 참조.
#   결과: 최적 69.0(det F1 0.9401, att F1 0.9337, FP 17 / FN 3).
#         반분 표본 최적이 전반 69 · 후반 69로 **완전 일치** → 잔여 불확실성 없음
#         (v2는 83·80으로 갈려 ±2를 기록해야 했다).
#   봉우리의 근거: 67.5로 내리면 FN 3→16(접촉이 offset 껍질에서 발화하므로 문턱이
#         낮으면 거리 시계열이 그 밑으로 안 내려가는 사건을 통째로 놓친다),
#         72로 올리면 FP 17→27(접촉 없는 근접을 충돌로 셈). 69가 교차점.
#   비율: τ/접촉거리 = 69/60 = **1.15**. v1(90/71.7=1.26)·v2(81/63.7=1.27)보다 낮은
#         것은 collider-trace-v1이 피벗-콜라이더 어긋남을 없앤 효과다. 남은 1.15배는
#         ① GT가 2r이 아니라 contact offset 껍질(실측 60.6~62.3)에서 발화하는 몫
#         ② 룰의 0.1초 스캔 격자가 최근접 순간을 놓치는 몫.
#   주의: 60.0(라벨러와 같은 술어)으로 반올림하지 않는다 — 스윕은 "룰에 가장 유리한
#         문턱을 clean에서 찾아 동결"하는 절차이므로 측정 최적을 쓰는 것이 룰에게
#         최선을 준 것이고, 이론값으로 되돌리면 오히려 불리하게 만든 셈이 된다.
# v2 규약(접촉거리 63.7) 기준은 81.0이었다(regime2 리포트의 수치는 이 값 기준).
FROZEN_TAU = 69.0
