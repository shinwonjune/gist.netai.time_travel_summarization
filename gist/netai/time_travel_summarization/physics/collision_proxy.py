...
# 1차 수정(지역 bbox 유도)은 실패했다: 5에피소드 진단에서 지역 치수가 84.9→131.1까지
# 변했는데, 131은 정적 단면(84.94x79.69)을 어떤 강체 회전으로 감싸도 넘을 수 없는
# 상한(대각선 116.4) 밖이다. 원인은 메시가 아니라 **API**였다(H5 — physics일지
# #17(e)·(f)에서 OpenUSD 소스까지 대조해 확정): kind 선언이 없으면 BBoxCache가 자식
# bound를 월드 공간에서 먼저 축 정렬 상자로 감싸고(이때 프림 회전이 값에 섞인다),
# 그 상자를 프림 변환의 역으로 되돌린 뒤 다시 축 정렬로 감싼다 — "기울어진 상자를
# 다시 상자로 감싼" 값이라 회전 각도에 따라 최대 2배까지 부푼다.
#   기각된 가설 둘(같은 진단에서): H3 "걷기 애니메이션 자세가 메시를 변형" —
#   이 에셋에는 애니메이션이 없고 자세가 고정이다(사용자 확인, 2026-08-09).
#   H4 "몸이 기울어진다" — world_up_in_prim_local이 전 기록 (0,0,±1) 상수.
# 캐시의 근거는 원인 규명과 **무관**하다. 필요한 전제는 "씬 로드 직후의 첫 측정은
# 오염 전"이라는 실측 하나뿐이고(진단②③ 모두 세션 첫 생성 회차 8/8건이 rest 값
# 84.94/35.86), 캐시는 그 첫 측정만 쓰고 이후에는 아예 재지 않는다. 지금은
# kind=component 선언(playback/stage_object_controller)이 오염원 자체를 없앴으므로
# 이 캐시는 이중 안전장치로 남는다.
_REST_DIMS: dict = {}
...
# - regime3: 0.9로 올렸다가(2026-08-16) 반동을 육안 확인한 뒤 **1.0으로 확정**
#   (사용자 결정, 커밋 bed622e에 포함). 충돌 사건이 화면에서 더 분명해지게 하려는
#   것이다. 0은 완전 비탄성, 1은 접촉에서 에너지 손실이 없는 값이며, 1 초과는
#   충돌마다 에너지가 늘어 발산하므로 쓰지 않는다. 1.0에서도 배회 컨트롤러가 매 틱
#   지시 속도를 다시 걸어 주므로 속도가 누적 증가하지는 않는다.
#   생산 데이터 prod-20260817-regime3-r1~r10은 이 1.0으로 생성됐다.
...
def wrap_with_collision_proxy(
    ...
    if shape in ("capsule", "cylinder") and bbox_valid:
        ...
        # 배회 진행 방향의 요(yaw)는 월드 상축을 중심으로 도는 회전이라 local_up을
        # 바꾸지 않는다 — 진단②③ 전 기록에서 (0,0,±1) 상수 실측(H4 tilt 기각 근거).
        ...
        # 프록시는 에피소드 경계마다 재생성되는데 그 시점의 진행 방향 요(yaw)가 남아 있어,
        ...
        if cached is not None:
            ...
            # 지금 다시 재면 그때까지 누적된 요(yaw)가 치수에 새어 든다(_REST_DIMS 주석).
